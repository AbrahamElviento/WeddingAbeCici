...
export default function App() {
  ...
  const [isCharmander, setIsCharmander] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const hasCharmanderKey = 
        name === 'charmander' ||
        params.get('secret') === 'charmander' ||
        params.get('key') === 'charmander' ||
        params.get('code') === 'charmander' ||
        params.has('charmander');

      if (hasCharmanderKey) {
        setIsCharmander(true);
      }

      if (name && name !== 'charmander') {
  ...
  return (
    <div className="min-h-screen font-sans bg-stone-50 text-stone-800 dark:bg-[#111312] dark:text-stone-200 transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="fixed bottom-6 left-6 z-40 flex items-center gap-2.5">
        ...
        {/* Floating Share Personal Link Generator Widget (Only rendered when secret "charmander" key is present in URL) */}
        {isCharmander && (
          <button 
            onClick={() => {
          ...
          <div className="relative z-10 w-full max-w-4xl my-auto py-8 animate-fade-in" style={{ animationDelay: '200ms' }}>
            ...
            {guestName && (
              <div className="mt-8 max-w-lg mx-auto px-6 py-5 bg-stone-900/40 dark:bg-black/50 backdrop-blur-md rounded-2xl border border-white/20 dark:border-white/15 shadow-2xl text-center animate-fade-in">
                ...
                {/* Personalized Link Generator (Only rendered when secret "charmander" key is present) */}
                {isCharmander && (
                  <button
                    type="button"
